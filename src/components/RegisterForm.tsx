  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const response = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ ...formState, age: parseInt(formState.age) }),
      });
      ...
      setHasError(err instanceof Error ? err.message : "登録に失敗しました");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 to-purple-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">アカウントを作成</h2>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="location" className="block text-sm font-medium text-gray-700">居住地</label>
              <input
                id="location"
                name="location"
                type="text"
          ...
          <div className="text-center">
            <a href="/login" className="text-pink-600 hover:text-pink-500">既にアカウントをお持ちですか？ログイン</a>
          </div>
        </form>
      </div>
    </div>
